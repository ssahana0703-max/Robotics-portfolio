...
class LunarGeoData(Dataset):
    ...
    def _load_legend(self) -> Dict[str, Dict[str, Union[str, List[int], int]]]:
        """Read the geomap legend from disk and enrich it with RGB values.

        Returns:
            Dict mapping class abbreviations (e.g. ``"Cc"``) to metadata dicts
            with keys ``"color"``, ``"rgb"``, ``"description"``.
        """
        legend_path = os.path.join(self.root, "UnifiedGeoMap", "legend.pkl")
        with open(legend_path, "rb") as f:
            raw_legend: Dict[str, Dict[str, Union[str, List[int]]]] = pickle.load(f)
    ...
    def compute_wac_stats(self) -> Tuple[float, float]:
        mean = self.wac.mean().item()
        std  = self.wac.std().item()
        print(f"WAC mean={mean:.4f}, std={std:.4f}")
        return mean, std
...
if __name__ == "__main__":
    ...
    dataset = LunarGeoData(root=root, patch_size=256, stride=128,transform=t)
    mean, std = dataset.compute_wac_stats()
    print(f"WAC mean={mean:.10f}, std={std:.10f}")
    print(f"Total number of patches: {len(dataset)}")
    sample = dataset[0]
    norm_patch = dataset[0]["wac"]["tensor"]
    print("\n--- AFTER Normalization ---")
    print(f"min:  {norm_patch.min():.8f}")
    print(f"max:  {norm_patch.max():.8f}")
    print(f"mean: {norm_patch.mean():.8f}")
    print(f"std:  {norm_patch.std():.8f}")

    print(
        f"WAC shape: {sample['wac']['tensor'].shape}, "
        f"Geomap shape: {sample['geomap']['tensor'].shape}"
    )
    # dataset = LunarGeoData(root="/path/to/data", patch_size=256, stride=128)
    # mean, std = dataset.compute_wac_stats()
    # print(f"WAC mean={mean:.4f}, std={std:.4f}")
    # # e.g. WAC mean=0.3821, std=0.1634
